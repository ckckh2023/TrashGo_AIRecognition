# TrashGo Smart Recognition Assistant

<img width="375" height="131" alt="TrashGo" src="https://github.com/user-attachments/assets/6f6b24e6-d62f-4ea6-9a77-030f98b35845" />

[![License](https://img.shields.io/badge/License-MIT-red?logo=opensourceinitiative&logoColor=white)](LICENSE)
[![Qt](https://img.shields.io/badge/Qt-6.10.3-brightgreen?logo=qt&logoColor=white)](https://www.qt.io/)
[![Platform](https://img.shields.io/badge/Platform-Windows%20%7C%20Linux-blue?logo=windows&logoColor=white)]()
[![GitHub Release](https://img.shields.io/github/v/release/ckckh2023/TrashGo_AIRecognition?include_prereleases&logo=github)](https://github.com/ckckh2023/TrashGo_AIRecognition/releases)
[![Stars](https://img.shields.io/github/stars/ckckh2023/TrashGo_AIRecognition?style=social&logo=github)]()

**Other languages:** [简体中文](README.md)

## Project Introduction

TrashGo is an intelligent tool that integrates multiple image information recognition capabilities, designed to help users quickly identify images, text, voice, and more, while providing an engaging and interactive experience. Whether it's waste sorting or face recognition, TrashGo handles it with ease.

## Features

- **Waste Sorting Recognition**: Take a photo to identify the waste category and get proper disposal guidance.
- ~~**Face Recognition**: Detect faces and analyze attributes such as age and expression.~~
- ~~**Fun Mini‑Games**: Waste sorting challenges, picture quizzes – learn while playing.~~
- **History**: Save all recognition records, support search, filter, and sharing.
- **Dark Mode**: Adapt to system dark theme for a more comfortable night‑time experience.
- ~~**More Settings**: Language switching, recognition precision adjustment, cache cleaning, etc.~~

## Screenshots

| Waste Sorting Recognition | History | Settings Page |
|:---:|:---:|:---:|
| <img width="962" height="631" alt="Waste Sorting Recognition" src="https://github.com/user-attachments/assets/a88edfd5-fdd8-4f0b-b11a-ed9e01667d40" /> | <img width="962" height="631" alt="History" src="https://github.com/user-attachments/assets/21e278bc-9ded-40b7-bb8f-572688643310" /> | <img width="962" height="631" alt="Settings Page" src="https://github.com/user-attachments/assets/a203451f-59b3-4329-989e-cebc0198eefe" /> |

## Installation Guide

### Download Installer
- **Windows/Linux**: Download the latest installer from [Releases](https://github.com/ckckh2023/TrashGo_AIRecognition/releases).

### Build from Source
1. Clone the repository: `git clone https://github.com/ckckh2023/TrashGo_AIRecognition.git`
2. Open the project (e.g., using Qt Creator).
3. Use the pre‑configured [CMakeLists.txt](CMakeLists.txt) to build and run.

## Tech Stack

- **Frontend**: Qt / Qml (cross‑platform)
- **Recognition Capabilities**: Local models & integrated third‑party AI APIs (e.g., Baidu AI, ~~Google ML Kit, TensorFlow Lite, etc.~~)
- **Database**: SQLite (local history storage)

## Contribution Guidelines

Contributions of code, bug reports, and feature suggestions are always welcome!

## License

This project is licensed under the MIT License – see the [LICENSE](LICENSE) file for details.

## Contact Us

- **Authors**: [ckckh2023](https://github.com/ckckh2023) | [TangibleDreams913](https://github.com/TangibleDreams913) | [cloudy-rhh](http://github.com/cloudy-rhh)
- **Email**: fightened2023@foxmail.com | Tangibledreams@yeah.net
- **Project Homepages**: [Github](https://github.com/ckckh2023/TrashGo_AIRecognition) | [Gitee](https://gitee.com/ckckh2023/TrashGo_AIRecognition) | [GitCode](https://gitcode.com/ckckh2023/TrashGo_AIRecognition)

## Acknowledgements

- Thanks to [Qt](https://www.qt.io/) for its excellent cross-platform framework.
- Thanks to [OpenCV](https://opencv.org/) for its powerful computer vision processing technology.
- Thanks to all contributors and supporters for their enthusiastic assistance.

## Star History

<a href="https://www.star-history.com/?repos=ckckh2023%2FTrashGo_AIRecognition&type=date&legend=top-left">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/chart?repos=ckckh2023/TrashGo_AIRecognition&type=date&theme=dark&legend=top-left" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/chart?repos=ckckh2023/TrashGo_AIRecognition&type=date&legend=top-left" />
   <img alt="Star History Chart" src="https://api.star-history.com/chart?repos=ckckh2023/TrashGo_AIRecognition&type=date&legend=top-left" />
 </picture>
</a>
